...
@st.cache_data(show_spinner=False)
def load_customers_with_segments():
    """
    Cleaned customer records (Module 6) with two extra columns attached:
    'Cluster' and 'Segment_Name'. Both are produced purely by calling
    .predict() on the saved production model against the existing,
    already-scaled engineered feature table -- the exact same inference
    used on the Customer Segmentation page. Nothing is fit or recomputed;
    this only lets Day-2's filters/search/charts work across the whole
    customer list instead of one row at a time.

    Returns None if the cleaned CSV or engineered feature CSV is missing.
    """
    cleaned_df = load_cleaned_customers()
    features_df = load_engineered_features()
    if cleaned_df is None or features_df is None:
        return None
    ...
    cluster_names_df = load_cluster_names()
    name_map = dict(zip(cluster_names_df["Cluster"], cluster_names_df["Segment_Name"]))
    ...
    merged["Segment_Name"] = merged["Cluster"].map(name_map)
    return merged
